...
const deletePost = async (req, res) => {
  ...
  const guideId = req.user.id; // From JWT token

  try {
    // Fetch post and associated images
    ...
    const images = await new Promise((resolve, reject) => {
      db.query('SELECT image FROM PostImages WHERE post_id = ?', [postId], (err, results) => {
        if (err) reject(err);
        else resolve(results);
      });
    });

    // Delete post (PostImages are deleted via ON DELETE CASCADE)
    await new Promise((resolve, reject) => {
...
const deleteGalleryImage = async (req, res) => {
  ...
  try {
    const [image] = await new Promise((resolve, reject) => {
      db.query('SELECT * FROM Gallery WHERE id = ? AND guide_id = ?', [imageId, guideId], (err, results) => {
        if (err) reject(err);
        else resolve(results);
      });
    });
    ...
    if (!image) {
      return res.status(404).json({ error: 'Gallery image not found or you are not authorized' });
    }
...
const getGuideProfile = async (req, res) => {
  ...
  try {
    const guideResults = await new Promise((resolve, reject) => {
      db.query(
    ...
    const galleryResults = await new Promise((resolve, reject) => {
      db.query(
