import React, { useState } from 'react';
   ...
   const GuideSignup = () => {
       ...
       const handleSubmit = async (e) => {
           ...
           try {
               const response = await fetch('http://localhost:5000/api/auth/guide', {
                   method: 'POST',
                   headers: {
                   ...
                   body: JSON.stringify({
                       name: form.name,
                       email: form.email,
                       national_id: form.guidenic,
                       contact: form.contact,
                       birthday: form.guidebirthday,
               ...
               if (response.ok) {
                   ...
                   setError(data.error || 'Failed to register guide');
                   toast.error(data.error || 'Failed to register guide');
               }
           } catch (error) {
               ...
               <div className="bg-white/80 backdrop-blur-md p-6 sm:p-8 rounded-lg shadow-2xl w-full max-w-4xl z-10">
                   ...
                   <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                       {[
                           ...
                           { label: 'National Identity Number', name: 'guidenic', type: 'text', placeholder: 'Enter NIC number' },
                           { label: 'Contact Number', name: 'contact', type: 'tel', placeholder: 'Enter contact number' },
                           { label: 'Date of Birth', name: 'guidebirthday', type: 'date', placeholder: 'Select birthday' },
                           ...
                           { label: 'Years of Experience', name: 'experience', type: 'number', placeholder: 'Enter years of experience' },
